...
{ 
    ...
    public class LSSLimit : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        private int lossSteps = 24; //the number of steps to recognize losses
        ...
        public override void OnData(Slice data)
        {
            if (data.Bars[symbol].Close > 0)
            {
                ...
                RealizeLosses(data.Bars[symbol]);
            }
        }
        ...
        private void RealizeLosses(TradeBar bar)
        {
            var openBuys = Transactions.GetOpenOrders(x => x.Direction == OrderDirection.Sell && x.Type == OrderType.Limit && x.Symbol == symbolName)
                    .OrderByDescending(x => Convert.ToInt32(x.Type));
            foreach(Order o in openBuys)
            {
                //check the price of the order minus the loss threshold, if it is greater than the current price, adjust the sell
                OrderTicket ot = openLimitOrders.Find(x => x.OrderId == o.Id);
                if (((1 - (lossSteps * stepPct)) * ot.Get(OrderField.LimitPrice)) > bar.Price)
                {
                    Debug(Time + " RealizeLosses " + ot.Get(OrderField.LimitPrice).ToString() + " -> " + (bar.Price * 1.002m).ToString());
                    //ot.Update(new UpdateOrderFields { LimitPrice = (bar.Price * 1.002m) });

                    //cancel and create market order
                    ot.Cancel();
                    MarketOrder(symbol, ot.Quantity - ot.QuantityFilled);
                }
            }
        }
